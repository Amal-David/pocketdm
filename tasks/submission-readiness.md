...
- Pikachu is persistent across the native macOS overlay, defaults to a pet-only animated state, uses four local 3D mood sprite strips, gives state-aware hints, includes daily Bond HP care, plays subtle original chirps, and switches between Pet/Happy, Nap, Hyper, and Alert reactions.
